...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      // getUser() ověří token na serveru — žádné false null sessions
      ...
      <div className="max-w-4xl mx-auto pt-12 space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl relative overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="text-5xl font-black mb-6 tabular-nums">{profile.credits_remaining.toLocaleString("cs-CZ")}</div>
          ...
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl relative overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="mt-auto">
              {profile.tier === 'free' ? (
                ...
                <button onClick={handlePortal} className="w-full inline-flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white py-3 px-4 rounded-xl font-bold transition-all border border-slate-700 hover:border-slate-500">
        ...
        {/* Fakturace — pouze pro placené tiery */}
        {profile.tier !== 'free' && (
          <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 text-center shadow-xl">
            ...
            <p className="text-slate-400 mb-6 text-sm">
              Potřebuješ stáhnout fakturu nebo změnit kreditní kartu? Vše najdeš v zákaznickém portálu Stripe.
            </p>
            <button onClick={handlePortal} className="bg-slate-800 hover:bg-slate-700 py-3 px-8 rounded-xl font-bold transition-all shadow-lg text-white">
