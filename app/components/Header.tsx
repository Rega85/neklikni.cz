...
import {
  ...
  CreditCard, User, Receipt, KeyRound, ExternalLink
} from "lucide-react";
    ...
    <header className="fixed top-0 left-0 right-0 z-[100] bg-slate-950/80 backdrop-blur-xl border-b border-white/5 h-16">
      <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Ceník — vždy viditelný */}
          <Link
            href="/pricing"
            className="text-slate-400 hover:text-white text-xs font-bold uppercase tracking-widest transition-colors hidden sm:block"
          >
            Ceník
          </Link>
          ...
          {loading ? (
            ...
            /* ── Přihlášený uživatel — dropdown menu ── */
                ...
                <div className="absolute right-0 top-12 w-72 bg-slate-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                  ...
                  <div className="p-4 border-b border-white/5">
                    ...
                    <Link
                      ...
                      className="mt-3 w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-xs font-black uppercase py-2 rounded-xl transition-colors"
                    >
            ...
            <div className="flex items-center gap-2">
              <Link
                href="/pricing"
                className="text-purple-400 hover:text-purple-300 text-xs font-black uppercase tracking-widest transition-colors sm:hidden"
              >
                Ceník
              </Link>
              <Link
                href="/login"
                className="bg-white text-black px-5 py-2 rounded-lg font-black text-xs hover:bg-slate-200 transition-colors"
              >
                PŘIHLÁSIT
              </Link>
            </div>
