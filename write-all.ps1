# write-all.ps1 - Zapise vsechny 3 soubory do projektu
...
'@
$header | Out-File -FilePath "app\components\Header.tsx" -Encoding UTF8 -NoNewline
Write-Host "OK: Header.tsx zapsan"
  ...
  const handleAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (!contentType?.includes("application/json")) {
        throw new Error("Server neodpovedel. Mozna je pretizen, zkuste to znovu.");
      }
      const data = await res.json();
      if (!res.ok) {
        if (res.status === 429 && data.limitReached) setError(data.message || "Denni limit vycerpan.");
        else if (res.status === 402) setError(data.message || "Nedostatek kreditu.");
        else setError(data.error || "Neco se pokazilo. Zkuste to znovu.");
        return;
      }
      setResult(data);
      ...
      setError(err.message || "Nepodarilo se pripojit k serveru.");
    } finally {
      setLoading(false);
    }
  }, [input, loading]);
        ...
        <div className="max-w-4xl w-full space-y-10 text-center">
          <div className="space-y-6">
            ...
            <h1 className="flex flex-col items-center justify-center font-black italic uppercase tracking-tighter">
              <span className="text-5xl sm:text-6xl md:text-7xl text-white leading-tight">PROVER</span>
          ...
          <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/10 rounded-[32px] shadow-2xl mx-auto max-w-3xl flex flex-col">
            <textarea
              ...
              placeholder="Vlozce podezrely text, SMS, email nebo URL..."
            ...
            <div className="flex items-center justify-between p-4 border-t border-white/5 bg-white/[0.02] rounded-b-[32px]">
              <span className="text-slate-600 text-[10px] hidden sm:block">Ctrl+Enter - Esc pro smazani</span>
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button onClick={handleAnalysis} disabled={loading || !input.trim()} className="bg-white text-black px-12 py-3 rounded-2xl font-black text-xs flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                  {loading ? <Loader2 className="animate-spin" size={18} /> : "PROVERIT"}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className={`rounded-[40px] border-2 backdrop-blur-3xl shadow-2xl overflow-hidden bg-slate-950/40 ${riskBorderColor} p-8 sm:p-10 text-left max-w-3xl mx-auto w-full`}>
              <div className="text-center mb-8">
                ...
                <div className={`inline-flex items-center gap-2 mb-3 ${riskColor}`}>
                  <RiskIcon size={20} />
                  <span className="font-black uppercase text-sm tracking-widest">{result.risk >= 70 ? "Vysoke riziko" : result.risk >= 40 ? "Stredni riziko" : "Nizke riziko"}</span>
                </div>
                <h2 className="text-xl sm:text-2xl font-black uppercase italic tracking-tighter text-white">{result.verdict}</h2>
              </div>
              <div className="space-y-6">
                <div className="space-y-2">
                  <h4 className="text-purple-400 font-black uppercase text-[10px] tracking-widest flex items-center gap-2"><Info size={14} /> Analyza</h4>
                  <p className="text-slate-300 text-sm leading-relaxed">{result.analysis}</p>
                </div>
                {result.threats && result.threats.length > 0 && (
                  <div className="space-y-2">
                    <h4 className="text-purple-400 font-black uppercase text-[10px] tracking-widest flex items-center gap-2"><AlertTriangle size={14} /> Identifikovane hrozby</h4>
                ...
                {result.tactics && result.tactics.length > 0 && (
                  <div className="space-y-2">
                    <h4 className="text-purple-400 font-black uppercase text-[10px] tracking-widest flex items-center gap-2"><Shield size={14} /> Taktiky utocnika</h4>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="text-slate-600 text-xs">{result.remainingChecks !== undefined && (<span>Zbyvá dnes: <span className="text-slate-400 font-bold">{result.remainingChecks}/3</span></span>)}</div>
                  <button onClick={handleShare} className="flex items-center gap-2 text-xs text-slate-400 hover:text-white transition-colors bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl">
                    {copied ? <><Check size={14} className="text-green-400" /> Skopirovano!</> : <><Share2 size={14} /> Sdilet varovani</>}
                  </button>
                </div>
              </div>
            </div>
      ...
      <footer className="w-full bg-[#020617] mt-auto border-t border-white/5 pt-8 pb-4">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-start justify-between gap-8 text-xs text-slate-500">
          <div className="space-y-3">
            ...
            <p>2026 Vsechna prava vyhrazena.</p>
          </div>
          <div className="flex flex-col md:flex-row gap-8 md:gap-16">
            <div className="space-y-1 leading-relaxed">
              ...
              <p>ICO: 21448507, DIC: CZ21448507</p>
              <p>Korunni 2569/108, Vinohrady, 101 00 Praha</p>
              <p>Datova schranka: bty8mey</p>
              <p>Spisova znacka: C 401405/MSPH Mestsky soud v Praze</p>
            </div>
            <div className="space-y-2 flex flex-col">
              <p className="text-slate-300 font-bold mb-1 uppercase text-[10px] tracking-widest">Informace</p>
              <Link href="/privacy" prefetch={false} className="hover:text-white transition-colors">Ochrana osobnich udaju</Link>
              <Link href="/terms" prefetch={false} className="hover:text-white transition-colors">Obchodni podminky</Link>
        ...
        <div className="max-w-7xl mx-auto px-6 mt-6 pt-4 border-t border-white/5 text-[10px] text-slate-600 text-center leading-relaxed">
          Vysledky analyzy vygenerovane umelou inteligenci maji informativni charakter. Technologie se muze mylit - posledni rozhodnuti je vzdy na Vas.
        </div>
      </footer>
    </div>
  );
...
'@
$page | Out-File -FilePath "app\page.tsx" -Encoding UTF8 -NoNewline
Write-Host "OK: page.tsx zapsan"
  ...
  if (!data?.profile) return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white">
      <div className="text-center space-y-4">
        <p className="text-slate-400">Nejste prihlaseni.</p>
        <Link href="/login" className="bg-white text-black px-6 py-2 rounded-lg font-black text-xs">PRIHLASIT SE</Link>
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-white pt-28 px-4 sm:px-6 pb-20">
      <div className="max-w-2xl mx-auto space-y-6">
        <h1 className="text-3xl font-black uppercase italic tracking-tighter">Fakturace a Predplatne</h1>
        <div className="bg-slate-900/40 border border-white/10 p-6 rounded-2xl space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Aktualni tarif</p>
              <p className="text-white font-black text-2xl">{tier.toUpperCase()}</p>
            </div>
            <CreditCard size={32} className="text-purple-500" />
          </div>
          <div>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Zbyvajici kredity</p>
        ...
        <Link href="/pricing" className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-black uppercase py-4 rounded-2xl transition-colors">
          <Zap size={16} fill="currentColor" /> Zmenit tarif nebo dobit
        </Link>
        <div className="bg-slate-900/40 border border-white/10 p-6 rounded-2xl flex flex-col items-center text-center space-y-4">
          <p className="text-slate-400 text-sm leading-relaxed">Faktury, historii plateb a nastaveni platebni karty spravujeme bezpecne pres sluzbu Stripe.</p>
...
'@
$billing | Out-File -FilePath "app\billing\page.tsx" -Encoding UTF8 -NoNewline
Write-Host "OK: billing/page.tsx zapsan"

Write-Host ""
Write-Host "Vsechny soubory zapsany. Spust: git add . && git commit -m fix && git push"
